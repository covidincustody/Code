...
# second install pytesseract and pillow
...
# Daily_Releases=re.findall(r"Total Daily Releases (\d*,?\d*)",text)[0]
# Total_transfer=re.findall(r"Total State Prison Inmates Awaiting Transfer (\d*,?\d*)",text)[0]

# pending_test=re.findall(r"PUls [(]Pending Test Result[)] (\d*,?\d*)",text)[0]
# Current_Total_transfer=re.findall(r"Total State Prison Inmates Awaiting Transfer (\d*,?\d*)",text)[0]
Current_Total_Confirmed=re.findall(r"Total Confirmed (\d*,?\d*)",text)[0]


# Positive_Current =re.findall(r"Current (\d*,?\d*)",text)[0]
# Positive_Released =re.findall(r"Released (\d*,?\d*)",text)[0]
# Positive_Negative_result =re.findall(r"Negative Test Results (\d*,?\d*)",text)[0]
# Positive_Historical =re.findall(r"Historical Positive Running Total (\d*,?\d*)",text)[0]
